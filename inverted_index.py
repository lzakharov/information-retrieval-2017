...
for i, article in enumerate(issue['articles']):
    words = filter(lambda w: w != '–',
                   map(lambda w: w.strip(punctuation), article['abstract_mystem'].split()))
    for word in words:
    ...
    words = filter(lambda w: w != '–',
                   map(lambda w: w.strip(punctuation), article['abstract_porter'].split()))
    for word in words:
    ...
    words = filter(lambda w: w != '–',
                   map(lambda w: w.strip(punctuation), article['title_mystem'].split()))
    for word in words:
    ...
    words = filter(lambda w: w != '–',
                   map(lambda w: w.strip(punctuation), article['title_porter'].split()))
    for word in words:
